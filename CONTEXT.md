...
A closed nominal SDK-issued capability for exactly one registered adapter-state maintenance action. The SDK-owned Source Application State Module binds its exact Topic, keyed engine mutation, and matching state transition when the adapter registers that state Module; an adapter cannot construct one from an ordinary Source Mutation or attach a user callback, Effect, Promise, I/O action, or structural lookalike. Runtime Core may execute it only through the Source Maintenance Interface, inside the same serialized uninterruptible critical section used for Source Delivery application. An internal maintenance-transition executor defect becomes the defective maintenance application Exit, invokes no Source Settlement, fails the complete View Server Runtime Effect, and relies on the surrounding `acquireUseRelease` or child Scope to release the lease.
...
The deep transport-neutral Interface nested in a Topic-Bound Source Toolkit for adapter-owned asynchronous correctness work that is not an external delivery lane. It accepts exactly one closed Source Maintenance Operation, applies its bound mutation and state transition through the ordinary engine path, and returns the complete application `Exit` without invoking Source Settlement or Source Supervision. Mutation success and its adapter-local transition complete in the same serialized uninterruptible critical section. It also accepts exact active/recovered Adapter Maintenance Failure transitions so Runtime Core can maintain the Source Status degradation ledger without inspecting adapter metrics. Failed work is an adapter-owned set keyed by stable work identity such as canonical ID plus generation: the first failure adds the item, repeated retry failures update cumulative attempt metrics without increasing backlog cardinality, and successful retry removes exactly that item. Interruption, retry suspension, or exhaustion does not remove a failed-work identity while its matching due generation remains indexed. A successful newer delivery that atomically replaces or removes that generation also cancels the now-stale identity; an explicit stale-generation observation may do the same. The marker clears only when this set is empty. Operational lifecycle status retains precedence as defined by Source Status. Exact adapter-specific failure details remain mandatory in Source Adapter Metrics and retain the latest safe maintenance failure for the logical source lifetime, replacing it only with a newer failure, so a failure followed by sub-cadence recovery still crosses at least one metrics publication. The Interface is scoped to the logical source lifetime and cannot query rows, accept ordinary mutations, bypass Topic validation, expose engine storage, or execute adapter code.
...
4. Launch new expiration attempts only while Source Status is `Ready` or `Degraded`. For a due entry, use `Effect.acquireUseRelease` (or an equivalent fresh child `Effect.scoped` acquisition) for the same lease, then recheck both the indexed generation and Source Status while holding the lease immediately before application. A stale candidate performs no row mutation, preserves the current newer indexed generation, and removes only the obsolete due entry and its exact failed-work identity. A status other than `Ready` or `Degraded` also performs no mutation but leaves the same due entry and failed-work identity intact for the next active sweep. No expiration lease is attached to the logical-lifetime sweep Scope.
...
- `Starting` is initial attempt `1n`; `Ready` begins immediately after successful Source Attempt acquisition only when the logical-lifetime degradation ledger is empty; `Degraded` contains that ledger's mandatory non-empty transport-neutral reason set with the latest exact settled Source Item Rejection and/or an Adapter Maintenance Failure marker; rejection degradation is sticky and maintenance degradation clears only when its backlog clears; `WaitingToRetry`, `Reacquiring`, `Exhausted`, and `Stopping` retain precedence while ledger transitions continue internally, and a later readiness transition selects `Degraded` when reasons remain.
