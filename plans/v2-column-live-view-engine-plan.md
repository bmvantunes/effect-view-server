...
- Delivery policy: every configured Region is an independent Source Delivery Lane, sibling lanes run concurrently, and records within one lane pass sequentially through the ordinary Source Adapter settlement contract. A valid record becomes one Source Delivery; an item-local decode, Mapping, canonical-ID validation, Topic Schema, or delete-only Local Row Key callback failure becomes one settled Source Item Rejection. Compact identity construction is SDK-owned from exact serialized key bytes and has no callback-failure phase. A successful Delivery or Rejection settlement commits that record's offset before the lane continues. Ordinary mutation-application or settlement failure preserves the record for replay and terminates the Source Attempt for ordinary supervision. An ordinary synchronous Source Settlement callback throw is caught and redacted to the exact SDK-owned `InvalidSourceSettlement` Source Runtime Failure before taking that same supervision path; it never escapes the lane or completes Runtime Fatal Signal. Lifecycle interruption is cancellation: it preserves the record but does not produce Source Termination or trigger a supervision retry. ADR 0011 is authoritative for invariant breaches: a closed Source Application Transition executor defect preserves the record, applies Source Settlement once through the bounded handoff using the outer delivery mask's restore, and completes the root Runtime Fatal Signal; a callback throw on that path is secondary only. A closed Source Maintenance Operation executor defect or unexpected sweep-fiber failure completes the same root signal without entering ordinary Source Supervision.
