...
View Server runs every Source Delivery in an attempt-owned interruptible application-and-settlement workflow rather than using Source Settlement as an `acquireUseRelease` release/finalizer. It captures the mutation application's complete Effect `Exit` and passes that value to Source Settlement. Successful application settles only after all mutations in that Source Delivery succeed; typed failure, defect, and cancellation remain distinguishable. Runtime Core applies the Source Settlement callback to that Exit exactly once to obtain an Effect whose typed failure belongs to the adapter's declared failure union and whose requirements remain visible in the View Server runtime Effect. The returned Effect is owned by the Source Attempt Scope, runs interruptibly, and must terminate promptly when that Scope closes; masking or ignoring Scope interruption is non-conformant. On ordinary application paths Runtime Core awaits settlement: success preserves the application outcome, while typed settlement failure becomes the operational Adapter Failure consumed by Source Supervision. Attempt-Scope interruption is lifecycle cancellation, produces no Source Termination, and triggers no retry. When ordinary application and settlement both fail, View Server records the original application Exit in spans and logs but does not invent a compound consumer-facing failure. A closed Source Application Transition defect instead follows ADR 0011's bounded ownership handoff, original-defect precedence, and root-fatal propagation without awaiting settlement completion. Already-applied mutations remain visible and are not rolled back. Adapters without external settlement semantics omit settlement and receive the SDK's infallible no-op settlement.
