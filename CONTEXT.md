...
**Runtime Heartbeat**:
The server-local periodic report of the View Server's current internal Source status. It reuses `Starting`, `Ready`, `Degraded`, `WaitingToRetry`, `Reacquiring`, `Exhausted`, and `Stopping`, and pairs that status with a canonical ordered problem-provenance list containing `self`, `dependency`, both, or neither. It is an application callback rather than a browser health protocol or active dependency probe.
_Avoid_: Invented healthy/unhealthy state machine, HTTP liveness endpoint, per-event health refresh

**Runtime Dependency Snapshot**:
The complete server-local inventory of configured upstream Dependency Targets. Every entry identifies its Source Adapter, logical target, exact configured endpoint strings, and existing Source Diagnostics status; configured leased targets without an active Feed use the existing `Inactive` state. Adapters own target discovery and failure classification, while Runtime Core performs transport-neutral aggregation.
_Avoid_: Changed entries only, transport-specific Runtime Core branches, sanitized endpoint alias, active probe result

**Failure Provenance**:
The classification of a current Source problem as `self` when View Server configuration, Mapping, mutation application, or internal runtime behavior failed, or `dependency` when acquisition, consumption, invocation, streaming, commit, release, unexpected upstream completion, or adapter-owned upstream payload decoding failed. SDK-owned runtime decoding and validation failures remain `self`; Kafka `KafkaDecodeFailure` is `dependency` because it describes an upstream record that the Kafka Adapter could not decode. An Adapter Failure is not inherently a dependency problem: each Source Adapter owns the pure classification and affected target identity.
_Avoid_: Treating every Adapter Failure as upstream, replacing Source Status, generic error-message parsing
...
**Live Query Viewport**:
The transport-neutral React integration Module for virtualized grids. It binds one typed Live Query, one inclusive absolute row window, and one caller-owned sparse row sink. React observes only query chrome; row payloads flow directly to the sink. Every full replacement and scroll-only window change has switch-latest ownership, so older snapshots, deltas, statuses, failures, and sink writes cannot mutate the current generation. Its opaque semantic key operation accepts the same exact query as replacement, reuses the replacement's owned snapshot and schema-aware identity, and gives equivalent fresh allocations one `Object.is`-equal key for the viewport lifetime without exposing canonical data. Its Viewport Source also supplies one topic-bound whole-result hook for independently scoped live projections such as facets; those subscriptions retain the topic's exact Route contract and never replace the sparse viewport generation.
...
**Kafka Schema Registry Resource**:
The Confluent Schema Registry runtime resource associated with one logical Kafka Region. When registry-backed decoding is configured, the Kafka Aggregate Node Layer constructs at most one registry client for that Region and shares it across every Topic and Source Attempt using the Region. The registry endpoint, authentication, TLS, and client are deployment resources, never browser-safe Source Definition data.
_Avoid_: Topic-owned registry, one registry per Topic, geographic region when meaning a broker/resource lane, raw registry URL in a Source Definition

**Kafka Schema Registry Protobuf Codec**:
A browser-safe Kafka Source Codec that explicitly opts a key or value into Confluent Schema Registry Protobuf decoding and requires a Buf-generated message descriptor as its exact type witness. One codec is valid in value, delete-only key, and metadata-free compaction-key positions because its decoded value depends only on the framed payload and generated descriptor; there is no separate compaction-key registry codec. The generated descriptor is mandatory; descriptor-free dynamic decoding and dynamic fallback are not supported. Protobuf is the only registry-backed format in the initial scope; Avro and JSON Schema are not supported. Configuring a Kafka Schema Registry Resource for a Region never changes the behavior of existing raw-payload bytes, string, JSON, Protobuf, or custom codecs.
_Avoid_: Compaction-specific registry codec, generic schema-registry codec, descriptor-free Protobuf, dynamic fallback, Avro support, JSON Schema support, implicit registry decoding, Region configuration that changes Source Definition semantics, unknown decoded value, transparent codec replacement
...
The mandatory exact policy on a first-party Kafka Source Definition that selects initial offsets independently in every selected Region and partition. Its five variants are `earliest`, `latest`, `committed`, absolute `timestamp` with epoch-nanosecond `bigint`, and `durationAgo` with Effect `Duration.Input`. `committed`, `timestamp`, and `durationAgo` carry a mandatory `earliest`, `latest`, or `fail` fallback for a partition without a usable offset. Relative time is read through Effect Clock and fixed once per Materialized Topic lifetime; the adapter converts nanoseconds to Kafka's millisecond timestamp resolution before using timestamp offset lookup and freezes the resolved initial partition offsets. A Source Attempt retry resumes the derived active consumer group's latest successfully committed offsets for the current lifetime and uses the frozen initial offset where that lifetime has not successfully committed the partition. If a consumer assignment discovers a partition absent from that frozen map, the Kafka Adapter makes bounded attempts to resolve and freeze the expanded map before terminating the Source Attempt with a typed Kafka Consume Failure, so ordinary metadata lag cannot move an effective `latest` boundary past newly appended records and persistent lag cannot leave health green indefinitely. If the discovery-time lookup omits that partition, any later resolution conservatively uses its earliest retained offset in place of the unavailable effective `latest` boundary. Existing uncommitted partitions retain their original frozen offsets even when a later metadata snapshot omits them, existing committed partitions resume active-group commits, and every newly discovered partition otherwise resolves the configured Kafka Start Position; `durationAgo` continues to use the same lifetime-fixed timestamp. When an already-pulled record first exposes a new partition resolved through `latest` or a `latest` missing-offset fallback, its offset becomes that partition's retry floor so reacquisition cannot advance past uncommitted live input. A complete runtime restart creates a new Materialized Topic lifetime and reevaluates `durationAgo`. Kafka v1 supports Materialized sources only, and there is no implicit start-position default.
...
- A **Live Query Viewport** keeps viewport rows out of React state, derives its query window, and gives every replacement or scroll-only window change switch-latest ownership over row, count, status, and failure delivery. Its opaque semantic key accepts exactly the replacement query, shares the replacement's owned snapshot and schema-aware identity, preserves exact Feed Route values, and remains stable across React activation and deactivation for the viewport lifetime. Its topic-bound whole-result hook preserves exact Feed Route values, forbids query windows, owns an independently scoped live subscription, and never replaces the sparse viewport generation. Its versioned, declaration-only **Base Topic Row Witness** invariantly identifies the complete configured Topic Row across raw projections, grouped results, window movement, source replacement, and downstream declaration bundling without adding runtime state. Its **Complete Raw Projection** is a stable source-owned field tuple that selects that exact row through the ordinary Raw Query path. Its declaration-only **Feed Route Witness** exposes the exact required leased Route object and extracts `never` for a materialized source; its **Where Witness** exposes source-native Filter Expressions over the complete Topic Row. The pure `effect-view-server/react/viewport-base-row` subpath directly owns all safe extractor declarations, imports neither React nor Effect, and emits an empty runtime module; the ordinary React surface reuses the source-owned extractors.
...
- `timestamp.atNanos` is a non-negative epoch-nanosecond `bigint`; `durationAgo.duration` is a finite non-negative Effect Duration input resolved from Effect `Clock.currentTimeMillis` wall time, never Date, `Date.now()`, or monotonic `Clock.monotonicTimeNanos`.
