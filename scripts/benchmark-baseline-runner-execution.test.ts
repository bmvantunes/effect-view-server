...
describe("benchmark baseline runner", () => {
  ...
  it("returns unknown profile failures", async () => {
    ...
    expect({
      ...
      exitCode: 1,
      message:
        "Unknown benchmark baseline profile: missing\nAvailable profiles: smoke, kafka-source-adapter, grpc-source-adapter, websocket-firehose, active-query-sharing, raw-read-write, grouped-admission, grouped-order-neutral, release",
    });
  });
