...
| Public package API examples in the roadmap                    | Implemented | `plans/v2-column-live-view-engine-plan.md` and `plans/grpc.md` now use public `effect-view-server/*` imports, `createViewServerReact(viewServer)`, `createInMemoryViewServerReact(viewServerReact)`, `runViewServerRuntime(viewServer, options)`, and explicit `select` in raw query examples. |
...
| ------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Earlier public API/runtime examples used obsolete package names, lazy Effect calls, and optional `totalRows` live-event fields. | Remove or rewrite | Reconciled with public `effect-view-server/*` imports, `Effect.runPromise` / `NodeRuntime.runMain` examples, required raw-query `select`, required `totalRows`, and current health hook return shape. |
