...
| In-memory browser/test provider                               | Implemented | `@effect-view-server/react/testing` and `@effect-view-server/in-memory`; browser tests and in-memory benchmarks use real runtime-core/engine.                                                                                                                                                                 |
...
| Current workspace API examples in the roadmap                 | Implemented | `plans/v2-column-live-view-engine-plan.md` and `plans/grpc.md` now use the internal `@effect-view-server/*` workspace scope, `createViewServerReact(viewServer)`, `createInMemoryViewServerReact(viewServerReact)`, `runViewServerRuntime(viewServer, options)`, and explicit `select` in raw query examples. |
...
| ------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Earlier public API/runtime examples used obsolete package names, lazy Effect calls, and optional `totalRows` live-event fields. | Remove or rewrite | Reconciled with implemented internal `@effect-view-server/*` workspace packages, `Effect.runPromise` / `NodeRuntime.runMain` examples, required raw-query `select`, required `totalRows`, and current health hook return shape. |
