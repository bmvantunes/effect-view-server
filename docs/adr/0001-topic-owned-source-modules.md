...
ADRs 0006–0010. Issue #385 implements first-party Kafka adoption. First-party
gRPC adoption and removal of the remaining transport-specific Topic source
properties stay staged in issues #386 and #387.
...
Kafka Delivery Contract stays local to Kafka source ingestion. Under the canonical model, Kafka tombstones can be interpreted using the topic-owned source declaration, decoded key, Region, metadata, canonical Topic Row ID, and target schema.
