...
**Source Status**:
The mandatory Schema-backed tagged union describing exactly one Source lifecycle state: `Starting`, `Ready`, `Degraded`, `WaitingToRetry`, `Reacquiring`, `Exhausted`, or `Stopping`. Each branch contains only its valid fields and nanosecond timestamps. Runtime Core owns a logical-lifetime degradation ledger with these invariants:
...
- `Degraded` means delivery continues while the ledger is non-empty and carries its mandatory non-empty transport-neutral reason set. Exact adapter-specific maintenance details remain in Source Adapter Metrics.
...
**Kafka Local Row Key**:
The transport-local string returned by a first-party Kafka Source Definition's `localRowKey` callback before the adapter constructs the canonical Topic Row ID. The Kafka adapter always combines the exact Region, Kafka partition, and local key as `region:partition:localRowKey`, injects that composite into `id`, and uses the same operation for Upserts and keyed Deletes. Region identifiers are non-empty and may not contain `:`; partition is the canonical non-negative decimal integer; local keys may contain `:` because decoding splits only at the first two separators. The adapter exposes canonical construction and decoding helpers. Partition is an adapter-owned identity component and is not exposed to `localRowKey`. This rule applies even to a one-region or one-partition source: Kafka ordering and compaction are partition-local, so independent clusters or partitions cannot silently collapse into one Topic Row.
_Avoid_: User-composed Kafka public key, `rowKey` callback, hidden region or partition namespace, delimiter without validation, region normalization

**Kafka Cleanup Policy**:
The mandatory browser-safe semantic declaration on every first-party Kafka Source Definition: `delete`, `compact`, or `compact-and-delete`. The aggregate Kafka Node Layer compares it during Kafka Broker Contract Validation with the effective `cleanup.policy` of the external topic in every selected Region. Broker text is interpreted as an unordered comma-separated set with insignificant surrounding whitespace, so `compact,delete`, `compact, delete`, and `delete,compact` are equivalent. A compaction-capable declaration requires a metadata-free Kafka Compaction Key Codec whose decode input contains only the non-null serialized key bytes; null keys are Source Item Rejections. Its `localRowKey` receives only that decoded key and Region, while the adapter adds partition to the canonical identity. Offset, timestamp, headers, partition, and other per-record metadata therefore cannot change the callback result or decoded key for identical key bytes. A `delete` declaration may use the ordinary Kafka key codec and additionally supplies the decoded non-null value to `localRowKey`; a null-valued record on that source is a Source Item Rejection rather than a tombstone.
_Avoid_: Unvalidated cleanup hint, ordered policy string, value-derived compacted key, delete-only tombstone

**Kafka Retention Policy**:
The mandatory browser-safe row-lifetime declaration on every first-party Kafka Source Definition: `match-kafka-retention` or an Effect `Duration.Input`. Matching Kafka retention resolves independently in every selected Region from the effective external-topic `retention.ms`; it retains forever for a compact-only topic or a Kafka value of `-1`, and otherwise uses the resolved non-negative duration. A matched zero keeps the universal deadline rule: deadline equals Kafka record timestamp plus zero, so a record is immediately eligible only when its timestamp is not later than the sweep's Effect Clock time; a future-dated record becomes eligible at its own timestamp. An explicit positive duration is authoritative local policy and may be shorter or longer than the broker value; Effect infinity explicitly retains forever, while explicit zero and negative durations are invalid configuration. The phrase “match Kafka retention” means matching configured time-based retention, not observing exact physical segment deletion or size-based `retention.bytes` behavior.
...
1. Before yielding an Upsert or any keyed Delete Source Delivery—including a tombstone or an already-expired non-null authoritative record—use `Effect.uninterruptibleMask` to acquire the canonical-ID permit and transfer its idempotent lease into an attempt-owned live-lease registry as one interruption-safe operation; registration failure releases the permit before restoring interruption. One Source Attempt Scope finalizer drains that registry, and ordinary release removes its entry immediately, so finalizer registrations and shutdown work are bounded by concurrent live leases rather than historical deliveries.
2. Runtime Core requires every transition-bearing retention delivery to contain exactly one mutation, rejecting multi-mutation shapes in public types and defensive runtime validation. It applies that singleton row mutation and executes its closed Source Application Transition in one serialized uninterruptible critical section. A successful Upsert replaces the indexed deadline from the Kafka record timestamp and cancels any failed-work identity for the replaced generation. Every successful keyed Delete—including a tombstone or already-expired non-null record—removes the matching deadline and failed-work identity. Rejection or failed application leaves the prior generation unchanged.
...
**Source Settlement**:
The adapter-owned callback for a Source Delivery, applied to the complete success, typed failure, defect, or cancellation `Exit` of View Server mutation application exactly once to obtain an Effect. The returned Effect's typed failure belongs to the adapter's declared failure union and its requirements remain visible. Its conformance contract requires prompt termination when the Source Attempt Scope interrupts it; masking or ignoring interruption is invalid. On ordinary application outcomes, Runtime Core captures the complete application `Exit`, applies the callback, and awaits the returned Effect in the attempt-owned interruptible workflow rather than an `acquireUseRelease` release/finalizer. Settlement success preserves the application outcome, while typed settlement failure becomes the operational Adapter Failure and the original application Exit remains diagnostic context rather than a fabricated compound Wire Protocol error. Attempt-Scope interruption is lifecycle cancellation, produces no Source Termination, and triggers no retry. On an internal transition-executor defect, Runtime Core uses one masked ownership handoff to register the supervised attempt child; the child's masked prefix applies the callback once, records that SDK-owned invocation marker, and signals the handoff latch; only then does Runtime Core complete the Runtime Fatal Signal without awaiting the returned Effect. The child restores that Effect interruptibly, so root Scope closure terminates even an interruptible `Effect.never`; arbitrary user code that masks or ignores interruption is explicitly non-conformant rather than something Runtime Core claims it can forcibly terminate.
...
- `Starting` is initial attempt `1n`; successful Source Attempt acquisition establishes operational readiness immediately, then the logical-lifetime degradation ledger selects visible `Ready` when empty or `Degraded` with its mandatory non-empty transport-neutral reason set when reasons remain. That set contains the latest exact settled Source Item Rejection and/or an Adapter Maintenance Failure marker; rejection degradation is sticky and maintenance degradation clears only when its backlog clears. `WaitingToRetry`, `Reacquiring`, `Exhausted`, and `Stopping` retain precedence while ledger transitions continue internally, and every later successful acquisition makes the same `Ready`-versus-`Degraded` selection.
...
- A Source Item Rejection or Adapter Maintenance Failure makes its exact Source Health, affected Topic health row, and aggregate View Server health summary degraded, while Live Query availability stays ready and later valid source items continue.
...
- The Kafka adapter validates each region as a non-empty string without `:`, validates Kafka partition as a non-negative integer, constructs every public Kafka-owned row key as `region:partition:localRowKey`, and exposes matching construction and decoding helpers.
- Kafka region text and local row-key text are preserved exactly without casing, accent, or whitespace transformation; the first two `:` separators delimit Region and canonical decimal partition, and a local row key may contain later colons.
...
- `compact` and `compact-and-delete` require a metadata-free Kafka Compaction Key Codec that decodes only non-null serialized key bytes, then restrict `localRowKey` to decoded key and Region; neither decoding nor the callback can observe decoded value or per-record metadata, while the adapter adds partition to canonical identity. `delete` supplies decoded key, decoded non-null value, and Region and treats a null-valued record as a Source Item Rejection rather than a tombstone.
...
- A matched broker `retention.ms = 0` is valid and sets each deadline to the Kafka record timestamp; the next sweep considers it due only when that timestamp is not in the future according to Effect Clock.
...
- Kafka retention uses idempotent canonical-ID leases across delivery application and sweep application. One attempt-scoped live-lease registry finalizer drains only currently held delivery leases; normal release removes entries immediately. The closed Source Application Transition atomically couples a successful singleton row mutation to its exact expiry-index transition and releases before external settlement; the registry is the fallback before transition. Each due sweep item uses `Effect.acquireUseRelease` or a fresh child Scope, then enters the Source Maintenance Interface while holding the lease. That Interface acquires the Source Lifecycle Gate and atomically linearizes status/generation admission, engine mutation, adapter state, failed-work state, and transport-neutral degradation-ledger transition; it cannot retain a lease in the logical-lifetime Scope.
...
- Exact Kafka Source Adapter Metrics expose declared and observed cleanup, configured and resolved retention, tracked rows, expired rows, already-expired authoritative Deletes, unique failed-work backlog, cumulative sweep retry failures, and last-sweep timing per Topic and Region. They retain the latest exact safe maintenance failure and cumulative failure state for the logical source lifetime; recovery clears active backlog/health state but not those diagnostics, and metric-only changes coalesce into the next ordinary one-second sample so sub-cadence failure and recovery appears in at least that next publication without an immediate health refresh.
...
- View Server applies each **Source Delivery**, captures its complete application `Exit`, and applies **Source Settlement** exactly once inside the Source Attempt Scope's interruptible workflow; settlement is not an `acquireUseRelease` release/finalizer.
