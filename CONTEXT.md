...
1. Before yielding an Upsert or tombstone Source Delivery, use `Effect.uninterruptibleMask` to acquire the canonical-ID permit and transfer its idempotent lease into an attempt-owned live-lease registry as one interruption-safe operation; registration failure releases the permit before restoring interruption. One Source Attempt Scope finalizer drains that registry, and ordinary release removes its entry immediately, so finalizer registrations and shutdown work are bounded by concurrent live leases rather than historical deliveries.
