...
**Live Query Viewport**:
The transport-neutral React integration Module for virtualized grids. It binds one typed Live Query, one inclusive absolute row window, and one caller-owned sparse row sink. React observes only query chrome; row payloads flow directly to the sink. Every full replacement and scroll-only window change has switch-latest ownership, so older snapshots, deltas, statuses, failures, and sink writes cannot mutate the current generation. Its opaque semantic key operation accepts the same exact query as replacement, reuses the replacement's owned snapshot and schema-aware identity, and gives equivalent fresh allocations one `Object.is`-equal key for the viewport lifetime without exposing canonical data.
