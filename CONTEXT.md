...
2. Let Source Settlement observe the complete application `Exit`. Only a successful Upsert Exit replaces the indexed deadline from the Kafka record timestamp; only a successful tombstone Exit removes it; rejection or failed application leaves the prior generation unchanged.
3. Release the lease before external settlement work. Attempt-scope closure is the fallback after pre-consumption or settlement interruption, so no permit survives retry.
4. For a due entry, acquire the same lease with `acquireRelease`, recheck its indexed generation, and hold the lease while the Source Maintenance Interface applies the ordinary Delete.
5. A successful Delete atomically removes the matching indexed generation while the lease is held. A stale generation changes nothing. A failed Delete leaves the generation indexed for retry and failed-backlog accounting.
