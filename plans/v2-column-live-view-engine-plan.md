...
> 0006–0011 supersede those shapes and failure-scope rules. Active work must use one canonical Topic
...
- Current workspace baseline: `effect@4.0.0-beta.106`.
...
Consumer code imports from the single public package, using stable
`effect-view-server/*` subpaths. The internal workspace package scope remains an
...
- Delivery policy: every configured Region is an independent Source Delivery Lane, sibling lanes run concurrently, and records within one lane pass sequentially through the ordinary Source Adapter settlement contract. A valid record becomes one Source Delivery; an item-local decode, Mapping, canonical-ID validation, Topic Schema, or delete-only Local Row Key callback failure becomes one settled Source Item Rejection. Compact identity construction is SDK-owned from exact serialized key bytes and has no callback-failure phase. Delivery and Rejection Settlement are synchronous callbacks receiving the complete application or rejection-recording `Exit`; Runtime Core applies either callback exactly once in the same bounded outer-mask workflow and runs only its returned Effect through that outer restore in the Source Attempt Scope. A successful Delivery or Rejection settlement commits that record's offset before the lane continues. A typed mutation-application or rejection-recording failure, a defect in ordinary row-mutation application, or a returned-settlement-Effect failure preserves the record for replay and terminates the Source Attempt for ordinary supervision. This classification explicitly excludes closed Source Application Transition and Source Maintenance Operation executor defects. Each attempt atomically arbitrates lifecycle cancellation against ordinary termination before any Source Termination/health/Schedule handoff. A synchronous settlement callback throw becomes exact redacted `InvalidSourceSettlement` under ordinary supervision only when ordinary termination wins. A cancellation request made after a non-cancelled operation `Exit` was captured but before that handoff still wins: callback application and failed-settlement accounting remain exactly once, while the redacted throw is secondary diagnostics with no Source Termination, health transition, Schedule step, retry/reacquisition, commit, or Runtime Fatal Signal. Interruption of an interruptible returned `Effect.never` follows that same cancellation rule. ADR 0011 is authoritative for invariant breaches: a closed Source Application Transition executor defect preserves the record, applies Source Settlement once through the bounded handoff using the outer delivery mask's restore, and completes the root Runtime Fatal Signal; a callback throw on that path is secondary only. A closed Source Maintenance Operation executor defect or unexpected sweep-fiber failure completes the same root signal without entering ordinary Source Supervision.
- Cancellation test policy: “no health transition” means no callback-throw-induced Source Supervision health transition. Delivery and rejection cases are asserted separately. A successfully recorded rejection preserves its `rejectedItemCount`, sticky rejection ledger, `Degraded` publication, and exact metrics subject to operational shutdown-status precedence while the callback throw causes no Source Termination, supervision health transition, Schedule step, retry/reacquisition, commit, or Runtime Fatal Signal.
