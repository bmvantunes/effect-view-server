    ...
    () => {
      ...
      writeFileSync(
        ...
        [
          ...
          "  topics: {",
          "    orders: {",
          "      schema: Schema.Struct({ id: ViewServerId, region: Schema.String, quantity: Schema.BigInt }),",
          '      source: sourceAdapter.leasedSource(["region"], undefined),',
          "    },",
          "  },",
          "});",
          "declare const alternateRouteDefinition: boolean;",
          "const alternateRouteConfig = defineViewServerConfig({",
          "  topics: {",
          "    orders: {",
          "      schema: Schema.Struct({ id: ViewServerId, region: Schema.String, desk: Schema.String }),",
          "      source: alternateRouteDefinition",
          '        ? sourceAdapter.leasedSource(["region"], undefined)',
          '        : sourceAdapter.leasedSource(["desk"], undefined),',
          "    },",
          "  },",
          "});",
          "const leasedOrMaterializedConfig = defineViewServerConfig({",
          "  topics: {",
          "    orders: {",
          "      schema: Schema.Struct({ id: ViewServerId, region: Schema.String }),",
          "      source: alternateRouteDefinition",
          '        ? sourceAdapter.leasedSource(["region"], undefined)',
          "        : sourceAdapter.materializedSource(undefined),",
          "    },",
          "  },",
          "});",
          ...
          'declare const alternateRouteOrders: LiveQueryViewport<typeof alternateRouteConfig.topics, "orders">;',
          ...
          'orderSource.useWholeResult({ select: ["id", "total"], where: [], orderBy: [{ field: "id", direction: "asc" }] });',
          'leasedOrderSource.useWholeResult({ routeBy: { region: "emea" }, select: ["id", "region"], where: [], orderBy: [{ field: "id", direction: "asc" }] });',
          "// @ts-expect-error a leased whole-result query requires its exact route.",
          'leasedOrderSource.useWholeResult({ select: ["id", "region"], where: [], orderBy: [{ field: "id", direction: "asc" }] });',
          "// @ts-expect-error a whole-result query cannot be paginated.",
          'orderSource.useWholeResult({ select: ["id"], where: [], orderBy: [], limit: 1 });',
          "const completeSelect: BundledViewportCompleteRawSelect<typeof orders> = orderSource.completeRawSelect;",
          'const leasedRoute: BundledViewportRouteBy<typeof leasedOrders> = { region: "emea" };',
          'const leasedWhere: BundledViewportWhere<typeof leasedOrders> = [{ field: "region", type: "contains", filter: "em" }];',
          'const leasedBigIntWhere: BundledViewportWhere<typeof leasedOrders> = [{ field: "quantity", type: "inRange", filter: 1n, filterTo: 3n }];',
          'const alternateRegionRoute: BundledViewportRouteBy<typeof alternateRouteOrders> = { region: "emea" };',
          'const alternateDeskRoute: BundledViewportRouteBy<typeof alternateRouteOrders> = { desk: "rates" };',
          "// @ts-expect-error alternate leased definitions never flatten into one combined Route.",
          'const combinedAlternateRoute: BundledViewportRouteBy<typeof alternateRouteOrders> = { region: "emea", desk: "rates" };',
          "// @ts-expect-error one exact alternate leased Route remains required.",
          "const missingAlternateRoute: BundledViewportRouteBy<typeof alternateRouteOrders> = {};",
          "type RejectLeasedMaterializedRoute = RequireNever<BundledViewportRouteBy<typeof leasedOrMaterializedOrders>>;",
          "type RejectMaterializedRoute = RequireNever<BundledViewportRouteBy<typeof orders>>;",
          "type RejectRouteAny = RequireNever<BundledViewportRouteBy<any>>;",
          "type RejectRouteUnknown = RequireNever<BundledViewportRouteBy<unknown>>;",
          "type RejectRouteUnwitnessed = RequireNever<BundledViewportRouteBy<{ readonly destroy: () => void }>>;",
          'type RejectRouteErased = RequireNever<BundledViewportRouteBy<LiveQueryViewport<any, string>>>;',
          "type RejectWhereAny = RequireNever<BundledViewportWhere<any>>;",
          "type RejectWhereUnknown = RequireNever<BundledViewportWhere<unknown>>;",
          "type RejectWhereUnwitnessed = RequireNever<BundledViewportWhere<{ readonly destroy: () => void }>>;",
          'type RejectWhereErased = RequireNever<BundledViewportWhere<LiveQueryViewport<any, string>>>;',
          'type CrossRowRouteWitness = Pick<typeof orders, "__effect-view-server/LiveQueryViewportBaseRow@v1"> & Pick<typeof leasedOrders, "__effect-view-server/LiveQueryViewportRouteBy@v1">;',
          'type CrossRowWhereWitness = Pick<typeof orders, "__effect-view-server/LiveQueryViewportBaseRow@v1"> & Pick<typeof leasedOrders, "__effect-view-server/LiveQueryViewportWhere@v1">;',
          "type RejectCrossRowRoute = RequireNever<BundledViewportRouteBy<CrossRowRouteWitness>>;",
          "type RejectCrossRowWhere = RequireNever<BundledViewportWhere<CrossRowWhereWitness>>;",
          "type RejectMixedRoute = RequireNever<BundledViewportRouteBy<typeof orders | typeof leasedOrders>>;",
          "type RejectMixedWhere = RequireNever<BundledViewportWhere<typeof orders | typeof leasedOrders>>;",
          "type RejectBroadRoute = RequireNever<BundledViewportRouteBy<Readonly<Record<string, (_row: Order) => Order>>>>;",
          "type RejectBroadWhere = RequireNever<BundledViewportWhere<Readonly<Record<string, (_row: Order) => Order>>>>;",
          "// @ts-expect-error a leased Route requires every exact Route Field.",
          "const missingLeasedRoute: BundledViewportRouteBy<typeof leasedOrders> = {};",
          "const extraLeasedRoute: BundledViewportRouteBy<typeof leasedOrders> = {",
          '  region: "emea",',
          "  // @ts-expect-error extra Route Fields are rejected after declaration bundling.",
          '  desk: "rates",',
          "};",
          "const wrongLeasedRoute: BundledViewportRouteBy<typeof leasedOrders> = {",
          "  // @ts-expect-error Route values retain their exact source domain.",
          "  region: 1,",
          "};",
          "const wrongLeasedWhere: BundledViewportWhere<typeof leasedOrders> = [",
          "  // @ts-expect-error known Fields retain their exact Filter operand domain.",
          '  { field: "region", type: "equals", filter: 1 },',
          "];",
          "const wrongBigIntWhere: BundledViewportWhere<typeof leasedOrders> = [",
          "  // @ts-expect-error numeric operands retain the source Field's exact domain.",
          '  { field: "quantity", type: "equals", filter: 1 },',
          "];",
          "const mixedBigIntRange: BundledViewportWhere<typeof leasedOrders> = [",
          "  // @ts-expect-error range bounds must remain in one exact numeric domain.",
          '  { field: "quantity", type: "inRange", filter: 1n, filterTo: 3 },',
          "];",
          "const unknownLeasedWhere: BundledViewportWhere<typeof leasedOrders> = [",
          "  // @ts-expect-error unknown Fields are rejected after declaration bundling.",
          ...
          "void leasedRoute;",
          "void leasedWhere;",
          "void leasedBigIntWhere;",
          "void alternateRegionRoute;",
          "void alternateDeskRoute;",
          "void combinedAlternateRoute;",
          "void missingAlternateRoute;",
          "void missingLeasedRoute;",
          "void extraLeasedRoute;",
          "void wrongLeasedRoute;",
          "void wrongLeasedWhere;",
          "void wrongBigIntWhere;",
          "void mixedBigIntRange;",
          "void unknownLeasedWhere;",
          "void completeField;",
          ...
          "void wrong;",
          "type Rejected = RejectAny | RejectCompleteAny | RejectCompleteUnknown | RejectCompleteUnwitnessed | RejectRouteAny | RejectRouteUnknown | RejectRouteUnwitnessed | RejectRouteErased | RejectLeasedMaterializedRoute | RejectWhereAny | RejectWhereUnknown | RejectWhereUnwitnessed | RejectWhereErased | RejectCrossRowRoute | RejectCrossRowWhere | RejectMixedRoute | RejectMixedWhere | RejectBroadRoute | RejectBroadWhere | RejectUnknown | RejectErasedViewport | RejectStringIndex | RejectPatternIndex | RejectUnsafeUnion | RejectMixedViewportUnion | RejectIntersectedViewports | RejectEquivalentViewportUnion | RejectNonInvariantWitness | RejectPartlyCallableWitness | RejectMixedCallableWitness | RejectOverloadedWitness | RejectUnwitnessed;",
          "",
        ].join("\n"),
      );
