...
1. Each Kafka Region flattens or `Stream.rechunk(1)`s every upstream transport batch into singleton record chunks before the first mapping stage that can acquire a canonical-ID permit. Mapping one singleton record may then acquire its permit and transfer its idempotent lease into the attempt-owned live-lease registry before yielding an Upsert or any keyed Delete Source Delivery—including a tombstone or an already-expired non-null authoritative record. This ordering prevents a mapper from retaining one record's lease while the same upstream chunk waits to acquire another lease for the same canonical ID. Use `Effect.uninterruptibleMask` for permit acquisition and registry transfer as one interruption-safe operation; registration failure releases the permit before restoring interruption. One Source Attempt Scope finalizer drains that registry, and ordinary release removes its entry immediately, so finalizer registrations and shutdown work are bounded by concurrent live leases rather than historical deliveries.
...
- Each Kafka Region flattens or `Stream.rechunk(1)`s upstream transport batches to singleton records before any mapping stage can acquire a canonical-ID permit, so two same-ID records from one transport chunk cannot self-deadlock while the first lease awaits downstream application.
