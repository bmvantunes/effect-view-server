...
**Source Status**:
The mandatory Schema-backed tagged union describing exactly one Source lifecycle state: `Starting`, `Ready`, `Degraded`, `WaitingToRetry`, `Reacquiring`, `Exhausted`, or `Stopping`. Each branch contains only its valid fields and nanosecond timestamps. `Degraded` means delivery continues despite one or more correctness problems and contains a mandatory non-empty transport-neutral degradation-reason set: the latest exact settled Source Item Rejection and/or an Adapter Maintenance Failure marker whose exact safe adapter-specific details remain in Source Adapter Metrics. A Source Item Rejection is sticky for the logical source lifetime because skipped input may leave the view incomplete. An Adapter Maintenance Failure means asynchronous adapter-owned maintenance, such as a Kafka retention Delete, failed without terminating ingestion; it clears only when the adapter reports that all corresponding failed work has subsequently succeeded or been cancelled. Runtime Core retains those independent reasons in a logical-lifetime ledger even while an operational lifecycle branch has precedence. `Starting`, `WaitingToRetry`, `Reacquiring`, `Exhausted`, and `Stopping` therefore keep their normal availability semantics; the next successful attempt becomes `Degraded` instead of `Ready` if the ledger remains non-empty. Waiting and reacquisition retain the exact Source Termination; exhaustion retains Source Retry Exhaustion; stopping names runtime shutdown or final Leased Feed release. React and Remote Browser Client APIs infer the complete union and narrow exhaustively on `_tag` without optional failure or retry fields. Live Query availability maps `Degraded` to ready while Source Diagnostics, Topic health, aggregate View Server health, and liveness/readiness payloads retain the distinction.
...
The deep transport-neutral Interface nested in a Topic-Bound Source Toolkit for adapter-owned asynchronous correctness work that is not an external delivery lane. It accepts only nominal Topic-bound mutations, applies them through the ordinary engine mutation path, and returns their complete application `Exit` without invoking Source Settlement or Source Supervision. It also accepts exact active/recovered Adapter Maintenance Failure transitions so Runtime Core can update its logical-lifetime degradation ledger without inspecting adapter metrics. The ledger records a sticky Source Item Rejection reason and an independently active/recovered Adapter Maintenance Failure reason during every Source Supervision phase. Operational lifecycle status has precedence: `Starting`, `WaitingToRetry`, `Reacquiring`, `Exhausted`, and `Stopping` never become `Degraded` merely because the ledger changes. A successful readiness transition publishes `Degraded` when the ledger is non-empty and `Ready` otherwise; clearing the last maintenance reason while ready returns to `Ready`, while a sticky rejection keeps it `Degraded`. This preserves Live Query availability while Topic health, aggregate View Server health, and separately subscribed Source Diagnostics receive the correct status transition. `Degraded` carries the ledger's non-empty Schema-backed transport-neutral reason set, and exact adapter-specific failure details remain mandatory in Source Adapter Metrics. The Interface is scoped to the logical source lifetime and cannot query rows, bypass Topic validation, or expose engine storage.
...
**Source No-Op Item**:
An external source item that an adapter intentionally maps to no View Server mutation, such as a heartbeat, adapter-level filter miss, or valid Kafka record whose retention deadline already passed before ingestion. The adapter settles it inside the pull-ordered Source Stream production effect before proceeding; settlement failure becomes an Adapter Failure and enters Source Supervision. A malformed, unparseable, or invalid item is instead a visible Source Item Rejection. The SDK defines no Skip mutation or empty Source Delivery, while a tombstone remains a real keyed Delete even when its Topic Row is already absent.
...
The best-effort local expiration of Kafka-owned Topic Rows according to their Kafka Retention Policy. The Kafka retention Module owns keyed serialization shared by delivery settlement and sweeping. Before yielding an Upsert or tombstone Source Delivery it acquires a canonical-ID permit as a scoped idempotent lease registered with the Source Attempt Scope. Source Settlement observes the complete application `Exit`, applies the successful deadline replacement or cancellation, and releases the lease before external settlement work; interruption before consumption or during settlement triggers the attempt-scope fallback release, so a permit cannot survive retry. Only a successful Upsert Exit replaces the deadline from the Kafka record timestamp; only a successful tombstone Exit cancels it; rejected or failed application leaves the previous deadline unchanged. A due sweep uses `acquireRelease` for the same keyed lease, rechecks the indexed generation, and holds it while the Source Maintenance Interface applies the ordinary Delete, so interruption always releases and an old deadline cannot interleave with and remove a newer row. An already-expired record is settled as a Source No-Op Item and never materializes. One logical Materialized Topic lifetime owns the keyed lock/index Module and a coarse Effect Clock sweep across Source Attempt retries, while individual delivery leases remain attempt-scoped. Failed expiration remains indexed for a later sweep, activates the transport-neutral Adapter Maintenance Failure reason, exposes the exact safe failure in Kafka Source Adapter Metrics, degrades Topic and aggregate View Server health, and never pauses or terminates Kafka ingestion. The reason clears only after the failed-expiration backlog reaches zero through successful retry or cancellation.
...
- `compact` and `compact-and-delete` require a Kafka key codec and exclude the decoded value from `localRowKey`; `delete` supplies the decoded non-null value and treats a null-valued record as a Source Item Rejection rather than a tombstone.
...
- A matched broker `retention.ms = 0` is valid and makes records immediately eligible for the next retention sweep.
...
- **Kafka Retention Projection** anchors deadlines to Kafka record timestamps, skips already-expired records without materializing them, replaces a row's deadline only after successful Upsert, and cancels it on tombstone Delete.
...
- Kafka retention uses scoped idempotent canonical-ID leases across delivery application/Exit handling and sweep application; normal settlement releases early after the Exit-driven state transition, Source Attempt closure releases an emitted-but-unconsumed or interrupted delivery, and sweep acquisition uses `acquireRelease`. Deadline replacement and cancellation depend on the successful application Exit, while a due sweep rechecks generation and holds the same lease through the Source Maintenance Interface's ordinary Delete.
- A failed retention Delete remains eligible for a later sweep, activates the transport-neutral Adapter Maintenance Failure reason in Source Status, publishes its exact safe detail in Kafka Source Adapter Metrics, degrades Topic and aggregate View Server health, and never pauses or terminates Kafka ingestion; the maintenance reason clears only after the failed backlog reaches zero.
- Exact Kafka Source Adapter Metrics expose declared and observed cleanup, configured and resolved retention, tracked rows, expired rows, already-expired no-ops, sweep failures, and last-sweep timing per Topic and Region.
...
- A **Source No-Op Item** is an intentional heartbeat, filter miss, or already-expired valid Kafka record settled by its adapter inside sequential Stream production before that source proceeds; it does not create an empty Source Delivery, Source Item Rejection, or shared Skip mutation.
